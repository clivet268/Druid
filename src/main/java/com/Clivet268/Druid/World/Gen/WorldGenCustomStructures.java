...
public class WorldGenCustomStructures implements IWorldGenerator {
    ...
    public static final SussyPiece DTOG = new SussyPiece(DRUID_TEMPLE_OG, 6);
    @Override
                ...
                if (random.nextInt(200) == 0) {
                    if (rprev <= 0) {
                        ...
                        for (int i = random.nextInt(5) + 2; i > 0; i--) {
                            ...
                            int xxx = xx + (ee * DH.siz);
                            eee = random.nextBoolean() ? 1 : -1;
                            int zz = (eee * (DT.siz + 3 + random.nextInt(19)));
                            int zzz = zz + (eee * DH.siz);
                            ...
                            while (((int) posise.distanceSq(new Vec3i(posespose.getX(), posise.getY() + 1, posespose.getZ()))) >= 64) {
                                ...
                                internalYfindingPos.add(0, getGroundFromAbove(world, internalYfindingPos.getX(), internalYfindingPos.getZ()), 0);
                                posise = internalYfindingPos;
    ...
    public static int getGroundFromAbove(World world, int x, int z){
        ...
        return MathHelper.clamp(y, 1,256);
