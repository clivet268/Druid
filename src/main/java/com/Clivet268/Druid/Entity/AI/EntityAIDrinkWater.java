        ...
        if (this.druid.getDistanceSq(bebpos) < 9.0D) {
            ...
            if (block == Blocks.CAULDRON) {
                ...
                for (int i = 6 + new Random().nextInt(12); i >= 0; i--) {
                    Minecraft.getMinecraft().effectRenderer.spawnEffectParticle(EnumParticleTypes.WATER_SPLASH.getParticleID(), this.druid.posX, this.druid.posY + 5, this.druid.posZ, new Random(69).nextDouble() / 4 * (new Random().nextBoolean() ? 1 : -1), new Random(69).nextDouble() / 8, new Random(69).nextDouble() / 4 * (new Random().nextBoolean() ? 1 : -1));
                }
                this.got = true;
            }
        } else {
